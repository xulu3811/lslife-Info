...
new_ui = """/** 多级分类级联选择 BottomSheet (Material 3 美化版) */
    ...
    ModalBottomSheet(
        ...
        Column(
            ...
            Row(
                ...
                Text("选择发布分类", fontSize = 20.sp, fontWeight = FontWeight.ExtraBold, color = MaterialTheme.colorScheme.onSurface)
                IconButton(onClick = onDismiss, modifier = Modifier.size(32.dp).background(MaterialTheme.colorScheme.surfaceVariant, CircleShape)) {
            ...
            OutlinedTextField(
                ...
                modifier = Modifier.fillMaxWidth().padding(bottom = 16.dp),
                placeholder = { Text("搜索分类，例: 手机 / 租房 / 兼职", fontSize = 14.sp) },
                leadingIcon = { Icon(Icons.Filled.Search, contentDescription = "Search", modifier = Modifier.size(20.dp)) },
                trailingIcon = {
                    if (searchQuery.isNotEmpty()) {
                        IconButton(onClick = { searchQuery = "" }) { Icon(Icons.Filled.Clear, contentDescription = "Clear", modifier = Modifier.size(20.dp)) }
                    }
                },
                singleLine = true,
                    ...
                    Row(modifier = Modifier.fillMaxSize()) {
                        ...
                        LazyColumn(
                            ...
                            subGroups.forEach { group ->
                                ...
                                item(key = "group_spacer_${group.id}") { Spacer(modifier = Modifier.height(16.dp)) }
                            }
                        }
                    }
                }
...
private fun CategoryListRow(
    ...
    Surface(
        ...
        color = if (isHot) Color(0xFFFFF3E0) else MaterialTheme.colorScheme.surfaceVariant.copy(alpha=0.5f),
